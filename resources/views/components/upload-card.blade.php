@props(['upload'])
<div class="w-2/3 sm:w-52 flex flex-col bg-gray-200 rounded-md shadow-md p-2 m-2 mx-auto">
    ...
    <div class="flex flex-col">
        ...
        <div class="flex">
            <div>
                <img src="{{ $upload->creator()->avatar }}" class="h-7 w-7 rounded-full border-solid border-2 border-gray-700" title="{{ $upload->creator()->name }}" />
            </div>
            <div class="text-xs flex flex-col justify-center items-center ml-1">
                <span title="{{ $upload->created_at }}">{{ $upload->created_at->diffForHumans(null, false, true) }}</span>
            </div>
